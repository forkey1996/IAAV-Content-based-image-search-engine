...
MODEL_NAME = '..\\Object Detection Google\\models\\research\\mask_rcnn_inception_resnet_v2_atrous_coco_2018_01_28'
...
def get_boxes_and_labels_on_image_array(
    ...
    for box, info_str in box_to_info_str_map.items():
        ...
        if instance_masks is not None:
            ...
            crop_image = get_mask_on_image_array(
                 crop_image,
                 mask
            )
...
plt.imshow(im_copy)
plt.axis('off')
plt.title('Mask R-CNN Object Detection Result')
plt.show()
...
for cropped_image in cropped_images:
    ...
    plt.imshow(im_crop)
    sub_plot.axis('off')
    sub_plot.set_title('Current object mask')
    idx = 0
